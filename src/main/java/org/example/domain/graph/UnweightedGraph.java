...
public class UnweightedGraph<V> implements Graph<V> {
    ...
    @Override
    /** Return the vertices in the graph */ public List<V> getVertices() {
        return vertices;
    }

    @Override
    /** Return the object for the specified vertex */ public V getVertex(int index) {
    ...
    @Override
    /** Return the neighbors of the specified vertex */ public List<Integer> getNeighbors(int index) {
    ...
    @Override
    /** Add an edge to the graph */ public boolean addEdge(Edge e) {
        if (e.u < 0 || e.u > getSize() - 1) throw new IllegalArgumentException("No such index: " + e.u);
        if (e.v < 0 || e.v > getSize() - 1) throw new IllegalArgumentException("No such index: " + e.v);
    ...
    @Override
    /** Obtain a DFS tree starting from vertex v */ public SearchTree dfs(int v) {
...
// Store the visited vertex
        ...
        for (Edge e : neighbors.get(v)) {// e.u is v
            if (!isVisited[e.v]) {// e.v is w in Listing 28.8
                ...
                dfs(e.v, parent, searchOrder, isVisited); // Rec
            }
        }
    }
    ...
    @Override
    /** Starting bfs search from vertex v */ public SearchTree bfs(int v) {
        List<Integer> searchOrder = new ArrayList<>();
        int[] parent = new int[vertices.size()];
        for (int i = 0; i < parent.length; i++)
            parent[i] = -1; // Initialize parent[i] to -1
        java.util.LinkedList<Integer> queue = new java.util.LinkedList<>(); // list used as a q
        boolean[] isVisited = new boolean[vertices.size()];
        ...
        while (!queue.isEmpty()) {
            ...
            for (Edge e : neighbors.get(u)) {// Note that e.u
                if (!isVisited[e.v]) {// e.v is w in Listing 28
...
// Display an edge
    ...
    @Override
    /** Remove vertex v and return true if successful */ public boolean remove(V v) {
        return true; // Implementation left as an exercise
    }

    @Override
    /** Remove edge (u, v) and return true if successful */ public boolean remove(int u, int v) {
        return true; // Implementation left as an exercise
    }
}
